...
test('frontend/dist/app.js and frontend/dist/app.min.js exist after build', (t) => {
    ...
    const appJsPath = path.resolve(frontendPath, 'dist/app.js');
    const appMinJsPath = path.resolve(frontendPath, 'dist/app.min.js');
...
test('frontend/dist/app.min.js imports @stefanobalocco/jtdal, not bare jtdal', (t) => {
    const appMinJsPath = path.resolve(__dirname, '../../frontend/dist/app.min.js');
    const content = fs.readFileSync(appMinJsPath, 'utf-8');
