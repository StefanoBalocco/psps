...
const { JSDOM } = _require( 'jsdom' ) as {
	JSDOM: new ( html?: string, options?: { url?: string; pretendToBeVisual?: boolean } ) => { readonly window: Record<string, unknown> };
};
...
type TemplateKey = 'accounts' | 'login';

// ---------------------------------------------------------------------------
// Helper: extract template string from frontend source
// ---------------------------------------------------------------------------

function extractTemplate( templateKey: TemplateKey ): string {
	const sourcePath: string = path.resolve( __dirname, '../../frontend/src/app.ts' );
	const source: string = fs.readFileSync( sourcePath, 'utf-8' );
	const regex: RegExp = new RegExp( `'${ templateKey }':[^\`]*\`([\\s\\S]*?)\`` );
	const match: Nullable<RegExpMatchArray> = regex.exec( source );
	if( null === match ) {
...
const accountsTemplate: string = extractTemplate( 'accounts' );
const loginTemplate: string = extractTemplate( 'login' );
...
test( 'www/ contains exactly app.min.js, index.html, style.css (no extra files)', ( t ) => {
	const wwwPath: string = path.resolve( __dirname, '../../www' );
	const entries: string[] = fs.readdirSync( wwwPath ).sort();
...
test( 'www has no .d.ts and no .map files', ( t ) => {
	const wwwPath: string = path.resolve( __dirname, '../../www' );
	const entries: string[] = fs.readdirSync( wwwPath );
	t.false( entries.some( ( e: string ) => e.endsWith( '.d.ts' ) ), 'www must not contain .d.ts files' );
	t.false( entries.some( ( e: string ) => e.endsWith( '.map' ) ), 'www must not contain .map files' );
} );
...
test( 'generated www/index.html exists', ( t ) => {
	const indexPath: string = path.resolve( __dirname, '../../www/index.html' );
...
test( 'www/index.html declares English document language', ( t ) => {
	const indexPath: string = path.resolve( __dirname, '../../www/index.html' );
	const html: string = fs.readFileSync( indexPath, 'utf-8' );
...
test( 'www/index.html contains app.min.js? with digits for cache busting', ( t ) => {
	const indexPath: string = path.resolve( __dirname, '../../www/index.html' );
	const html: string = fs.readFileSync( indexPath, 'utf-8' );
	const match: Nullable<RegExpMatchArray> = html.match( /app\.min\.js\?(\d+)/ );
...
test( 'www/index.html contains style.css? with digits for cache busting', ( t ) => {
	const indexPath: string = path.resolve( __dirname, '../../www/index.html' );
	const html: string = fs.readFileSync( indexPath, 'utf-8' );
	const match: Nullable<RegExpMatchArray> = html.match( /style\.css\?(\d+)/ );
...
test( 'www/app.min.js contains App module export and start call', ( t ) => {
	const appMinJsPath: string = path.resolve( __dirname, '../../www/app.min.js' );
	const content: string = fs.readFileSync( appMinJsPath, 'utf-8' );
	t.true( -1 !== content.indexOf( 'class App' ) || -1 !== content.indexOf( 'export class App' ),
		'www/app.min.js must contain App class' );
...
test( 'frontend source is app.ts; psps.ts does not exist', ( t ) => {
	const appSourcePath: string = path.resolve( __dirname, '../../frontend/src/app.ts' );
	const pspsSourcePath: string = path.resolve( __dirname, '../../frontend/src/psps.ts' );
...
test( 'app.ts exports class App', ( t ) => {
	const sourcePath: string = path.resolve( __dirname, '../../frontend/src/app.ts' );
	const src: string = fs.readFileSync( sourcePath, 'utf-8' );
...
test( 'app.ts uses App static references (not PSPS)', ( t ) => {
	const sourcePath: string = path.resolve( __dirname, '../../frontend/src/app.ts' );
	const src: string = fs.readFileSync( sourcePath, 'utf-8' );
	const codeOnly: string = src.replace( /`[^`]*`/g, '' ); // Remove template literals which contain brand text
...
test( 'index.html importmap resolves to scoped packages with @latest versions', ( t ) => {
	const indexPath: string = path.resolve( __dirname, '../../frontend/index.html' );
	const html: string = fs.readFileSync( indexPath, 'utf-8' );
...
test( 'login template first OTP has autocomplete one-time-code, others do not', ( t ) => {
	...
	for( let iL1: number = 1; iL1 < cL1; iL1++ ) {
		const fieldId: string = `login-pin-${ iL1 }`;
		const fieldStart: number = loginTemplate.indexOf( fieldId );
		const beforeField: string = loginTemplate.substring( Math.max( 0, fieldStart - 80 ), fieldStart );
...
test( 'accounts template renders without placeholders or data-tdal remnants', ( t ) => {
	...
	const result: string = engine.CompileToFunction( accountsTemplate )( data );
...
test( 'accounts template has four labelled rows in strict order', ( t ) => {
	const serviceIdx: number = accountsTemplate.indexOf( 'Service' );
	const usernameIdx: number = accountsTemplate.indexOf( 'Username' );
	const passwordIdx: number = accountsTemplate.indexOf( 'Password' );
	const linkIdx: number = accountsTemplate.indexOf( 'Link' );
...
test( 'accounts template all SVGs have aria-hidden and focusable false', ( t ) => {
	const svgCount: number = ( accountsTemplate.match( /<svg/g ) || [] ).length;
	const ariaHiddenCount: number = ( accountsTemplate.match( /aria-hidden="true"/g ) || [] ).length;
	const focusableCount: number = ( accountsTemplate.match( /focusable="false"/g ) || [] ).length;
...
test( 'login template renders with pin step showing masked email and timer', ( t ) => {
	...
	const result: string = engine.CompileToFunction( loginTemplate )( data );
...
test( 'frontend/index.html uses jsdelivr for chota (not unpkg)', ( t ) => {
	const indexPath: string = path.resolve( __dirname, '../../frontend/index.html' );
	const html: string = fs.readFileSync( indexPath, 'utf-8' );
	t.true( -1 !== html.indexOf( 'https://cdn.jsdelivr.net/npm/chota@0.9.2/dist/chota.min.css' ),
		'frontend/index.html must use jsdelivr for chota' );
	t.false( -1 !== html.indexOf( 'https://unpkg.com/chota@0.9.2/dist/chota.min.css' ),
		'frontend/index.html must not use unpkg for chota' );
} );

test( 'built www/index.html uses jsdelivr for chota (not unpkg)', ( t ) => {
	const indexPath: string = path.resolve( __dirname, '../../www/index.html' );
	const html: string = fs.readFileSync( indexPath, 'utf-8' );
	t.true( -1 !== html.indexOf( 'cdn.jsdelivr.net/npm/chota@0.9.2/dist/chota.min.css' ),
		'www/index.html must use jsdelivr for chota' );
	t.false( -1 !== html.indexOf( 'unpkg.com/chota@0.9.2/dist/chota.min.css' ),
		'www/index.html must not use unpkg for chota' );
} );

test( 'frontend/index.html uses jsdelivr for @stefanobalocco/jtdal (not unpkg)', ( t ) => {
	const indexPath: string = path.resolve( __dirname, '../../frontend/index.html' );
	const html: string = fs.readFileSync( indexPath, 'utf-8' );
	t.true( -1 !== html.indexOf( 'https://cdn.jsdelivr.net/npm/@stefanobalocco/jtdal@latest/jTDAL.min.js' ),
		'frontend/index.html import map must use jsdelivr for @stefanobalocco/jtdal' );
	t.false( -1 !== html.indexOf( 'https://unpkg.com/@stefanobalocco/jtdal@latest/jTDAL.min.js' ),
		'frontend/index.html import map must not use unpkg for @stefanobalocco/jtdal' );
} );

test( 'built www/index.html uses jsdelivr for @stefanobalocco/jtdal (not unpkg)', ( t ) => {
	const indexPath: string = path.resolve( __dirname, '../../www/index.html' );
	const html: string = fs.readFileSync( indexPath, 'utf-8' );
	t.true( -1 !== html.indexOf( 'cdn.jsdelivr.net/npm/@stefanobalocco/jtdal@latest/jTDAL.min.js' ),
		'www/index.html import map must use jsdelivr for @stefanobalocco/jtdal' );
	t.false( -1 !== html.indexOf( 'unpkg.com/@stefanobalocco/jtdal@latest/jTDAL.min.js' ),
		'www/index.html import map must not use unpkg for @stefanobalocco/jtdal' );
} );
...
test( 'frontend/style.css has dark mode media query', ( t ) => {
	const stylesPath: string = path.resolve( __dirname, '../../frontend/style.css' );
	const content: string = fs.readFileSync( stylesPath, 'utf-8' );
	t.true( -1 !== content.indexOf( '@media ( prefers-color-scheme: dark )' ),
		'frontend/style.css must contain dark mode media query' );
	t.true( -1 !== content.indexOf( '--color-darkGrey' ),
		'frontend/style.css must use --color-darkGrey' );
	t.false( -1 !== content.indexOf( '--color-grey-dark' ),
		'frontend/style.css must not use --color-grey-dark' );
} );

test( 'built www/style.css has dark mode media query', ( t ) => {
	const stylesPath: string = path.resolve( __dirname, '../../www/style.css' );
	const content: string = fs.readFileSync( stylesPath, 'utf-8' );
	t.true( -1 !== content.indexOf( '@media ( prefers-color-scheme: dark )' ),
		'www/style.css must contain dark mode media query' );
} );
...
test( 'frontend/style.css has reduced motion media query', ( t ) => {
	const stylesPath: string = path.resolve( __dirname, '../../frontend/style.css' );
	const content: string = fs.readFileSync( stylesPath, 'utf-8' );
	t.true( -1 !== content.indexOf( 'prefers-reduced-motion: reduce' ),
		'CSS must contain reduced motion media query' );
} );
...
test( 'frontend/style.css dark mode overrides psps tokens', ( t ) => {
	...
	const darkModeSection: string = content.substring( content.indexOf( '@media ( prefers-color-scheme: dark )' ) );
...
test( 'frontend/style.css has body with warm paper and ink colors', ( t ) => {
	const stylesPath: string = path.resolve( __dirname, '../../frontend/style.css' );
	const content: string = fs.readFileSync( stylesPath, 'utf-8' );
	t.true( -1 !== content.indexOf( 'background: var( --psps-paper )' ) || -1 !== content.indexOf( 'background: var(--psps-paper)' ),
		'Body must use psps-paper background' );
	t.true( -1 !== content.indexOf( 'color: var( --psps-ink )' ) || -1 !== content.indexOf( 'color: var(--psps-ink)' ),
		'Body must use psps-ink color' );
} );

test( 'frontend/style.css #app overrides Chota container with full-bleed', ( t ) => {
	const stylesPath: string = path.resolve( __dirname, '../../frontend/style.css' );
	const content: string = fs.readFileSync( stylesPath, 'utf-8' );
	const appSection: string = content.indexOf( '#app' ) >= 0 ? content.substring( content.indexOf( '#app' ) ) : '';
...
test( 'frontend/style.css login-shell uses 100dvh and no outer padding on mobile', ( t ) => {
	...
	const loginShellStart: number = content.indexOf( '.login-shell' );
	const loginShellBlock: string = content.substring( loginShellStart, content.indexOf( '}\n\n.login-panel' ) );
...
test( 'frontend/style.css login panel is full-width flat on mobile, restored at 48rem', ( t ) => {
	...
	const panelStart: number = content.indexOf( '.login-panel' );
	const panelBlock: string = content.substring( panelStart, panelStart + 300 );
...
test( 'frontend/style.css accounts-list has no min-height', ( t ) => {
	const stylesPath: string = path.resolve( __dirname, '../../frontend/style.css' );
	const content: string = fs.readFileSync( stylesPath, 'utf-8' );
	const start: number = content.indexOf( '.accounts-list' );
	t.true( -1 !== start, 'CSS must contain .accounts-list selector' );
	const block: string = content.substring( start, content.indexOf( '@media', start ) );
...
test( 'frontend/style.css .account-card .link-external has flex:1, truncation properties', ( t ) => {
	const stylesPath: string = path.resolve( __dirname, '../../frontend/style.css' );
	const content: string = fs.readFileSync( stylesPath, 'utf-8' );
	const start: number = content.indexOf( '.account-card .link-external' );
	t.true( -1 !== start, 'CSS must contain .account-card .link-external' );
	const block: string = content.substring( start, content.indexOf( '}', start ) );
...
test( 'frontend/style.css dark mode keeps --color-darkGrey and warm primary', ( t ) => {
	const stylesPath: string = path.resolve( __dirname, '../../frontend/style.css' );
	const content: string = fs.readFileSync( stylesPath, 'utf-8' );
	const darkModeSection: string = content.indexOf( '@media ( prefers-color-scheme: dark )' ) >= 0
...
test( 'frontend/style.css reduced motion includes icon-button', ( t ) => {
	const stylesPath: string = path.resolve( __dirname, '../../frontend/style.css' );
	const content: string = fs.readFileSync( stylesPath, 'utf-8' );
	const reducedSection: string = content.indexOf( 'prefers-reduced-motion: reduce' ) >= 0
...
test( 'frontend/style.css field-value has min-width: 0 and overflow-wrap: anywhere', ( t ) => {
	const stylesPath: string = path.resolve( __dirname, '../../frontend/style.css' );
	const content: string = fs.readFileSync( stylesPath, 'utf-8' );
	const start: number = content.indexOf( '.account-card .field-value' );
...
test.serial( 'full user journey: email validation, OTP, resend, verification, accounts, toggle, copy', async ( t ) => {
	...
	const doc: Document = dom.window.document as unknown as Document;
	const DomEvent: typeof Event = dom.window.Event as unknown as typeof Event;

	// ---- Save original global descriptors ----
	const savedDescriptors: Record<string, Nullable<TypedPropertyDescriptor<unknown>>> = {};
	const cGlobals: number = _runtimeGlobals.length;
	for( let iL1: number = 0; iL1 < cGlobals; iL1++ ) {
		const name: string = _runtimeGlobals[ iL1 ];
		savedDescriptors[ name ] = Object.getOwnPropertyDescriptor( globalThis, name ) || null;
	}

	// ---- Install JSDOM globals ----
	( globalThis as Record<string, unknown> ).document = dom.window.document as unknown as Document;
	...
	( globalThis as Record<string, unknown> ).Event = DomEvent;
	...
	let testNow: number = originalDateNow();
	Date.now = (): number => testNow;

	// ---- Teardown: restore globals, Date.now, and close JSDOM ----
	const jsdomWindow: Window = dom.window as unknown as Window;
	t.teardown( (): void => {
		Date.now = originalDateNow;
		if( !jsdomWindow.closed ) {
			jsdomWindow.close();
		}
		const cRestore: number = _runtimeGlobals.length;
		for( let iL1: number = 0; iL1 < cRestore; iL1++ ) {
			const name: string = _runtimeGlobals[ iL1 ];
			const descriptor: Nullable<TypedPropertyDescriptor<unknown>> = savedDescriptors[ name ];
			if( null !== descriptor ) {
	...
	let emailFetchCount: number = 0;
	let verifyFetchCount: number = 0;
	const fetchRequests: { url: string; method: string; body: Record<string, unknown> }[] = [];
	...
	( globalThis as Record<string, unknown> ).fetch = async (
		...
		const urlStr: string = 'string' === typeof url ? url : url.toString();
		if( '/api/login' === urlStr && 'POST' === ( init?.method || 'GET' ) ) {
			...
			if( body.email && !body.pin ) {
				const idx: number = emailFetchCount++;
				switch( idx ) {
			...
			if( body.email && body.pin ) {
				const idx: number = verifyFetchCount++;
				switch( idx ) {
	...
	const originalClipboard: unknown = ( navigator as unknown as Record<string, unknown> ).clipboard;
	Object.defineProperty( navigator, 'clipboard', {
		value: { writeText: async ( text: string ): Promise<void> => { clipboardText = text; } },
		writable: true,
		configurable: true,
	} );
	t.teardown( (): void => {
	...
	let emailField: Nullable<HTMLInputElement> = doc.getElementById( 'login-email' ) as Nullable<HTMLInputElement>;
	...
	let formEl: Nullable<HTMLElement> = doc.querySelector( '[data-myopie="emailForm"]' ) as Nullable<HTMLElement>;
	t.truthy( formEl, 'Email form must be present' );
	const prevEmailCount: number = emailFetchCount;
	...
	emailField = doc.getElementById( 'login-email' ) as Nullable<HTMLInputElement>;
	t.truthy( emailField, 'Email input must exist for step 2' );
	emailField!.value = 'user@test.com';
	formEl = doc.querySelector( '[data-myopie="emailForm"]' ) as Nullable<HTMLElement>;
	t.truthy( formEl, 'Email form must exist for step 2' );
	formEl!.dispatchEvent( new DomEvent( 'submit', { bubbles: true, cancelable: true } ) );
	await new Promise( ( resolve: ( value: void ) => void ): void => { setTimeout( resolve, 250 ); } );
	...
	const pin0: Nullable<HTMLInputElement> = doc.getElementById( 'login-pin-0' ) as Nullable<HTMLInputElement>;
	t.truthy( pin0, 'OTP field 0 must exist' );
	pin0!.value = '5';
	pin0!.focus();
	await new Promise( ( resolve: ( value: void ) => void ): void => { setTimeout( resolve, 50 ); } );
	// ponytail: JSDOM focus does not bubble; select() only runs if event wired
	if( 0 === pin0!.selectionStart ) {
		t.true( 0 < ( pin0!.selectionEnd as number ), 'Selection end must cover content if select() ran' );
	}
	// Plain native paste event (no clipboardData)
	const pasteEvent: Event = new DomEvent( 'paste', { bubbles: true, cancelable: true } );
	pin0!.dispatchEvent( pasteEvent );
	await new Promise( ( resolve: ( value: void ) => void ): void => { setTimeout( resolve, 30 ); } );
	...
	let pinField: Nullable<HTMLInputElement> = doc.getElementById( 'login-pin-2' ) as Nullable<HTMLInputElement>;
	...
	const cleared0: Nullable<HTMLInputElement> = doc.getElementById( 'login-pin-0' ) as Nullable<HTMLInputElement>;
	...
	pinField = doc.getElementById( 'login-pin-2' ) as Nullable<HTMLInputElement>;
	t.truthy( pinField, 'OTP field 2 must exist for step 7' );
	pinField!.value = '123456';
	pinField!.dispatchEvent( new DomEvent( 'input', { bubbles: true } ) );
	await new Promise( ( resolve: ( value: void ) => void ): void => { setTimeout( resolve, 300 ); } );
	const errorBanner: Nullable<HTMLElement> = doc.querySelector( '.error-banner' ) as Nullable<HTMLElement>;
	...
	const cleared1: Nullable<HTMLInputElement> = doc.getElementById( 'login-pin-0' ) as Nullable<HTMLInputElement>;
	...
	let emailField2: Nullable<HTMLInputElement> = doc.getElementById( 'login-email' ) as Nullable<HTMLInputElement>;
	t.truthy( emailField2, 'Email input must exist after reset' );
	emailField2!.value = 'user@test.com';
	let formEl2: Nullable<HTMLElement> = doc.querySelector( '[data-myopie="emailForm"]' ) as Nullable<HTMLElement>;
	...
	testNow += 3000;
	await new Promise( ( resolve: ( value: void ) => void ): void => { setTimeout( resolve, 1100 ); } );
	let resendBtn: Nullable<HTMLElement> = doc.querySelector( '[data-myopie="resendPin"]' ) as Nullable<HTMLElement>;
	t.truthy( resendBtn, 'Resend button must be enabled after timer expiry' );

	// ============ 11. Resend with non-OK response ============
	const prevReqsLen1: number = fetchRequests.length;
	...
	let resendAfter1: Nullable<HTMLElement> = doc.querySelector( '[data-myopie="resendPin"]' ) as Nullable<HTMLElement>;
	t.truthy( resendAfter1, 'Resend button must remain after non-OK resend' );

	// ============ 12. Resend with rejected fetch ============
	const prevReqsLen2: number = fetchRequests.length;
	...
	const resendAfter2: Nullable<HTMLElement> = doc.querySelector( '[data-myopie="resendPin"]' ) as Nullable<HTMLElement>;
	t.truthy( resendAfter2, 'Resend button must remain after rejected resend' );

	// ============ 13. Complete OTP → verify success → accounts ============
	const pin2Final: Nullable<HTMLInputElement> = doc.getElementById( 'login-pin-2' ) as Nullable<HTMLInputElement>;
	...
	const toggleButtons: NodeListOf<HTMLButtonElement> = doc.querySelectorAll( '[data-myopie="togglePassword"]' ) as NodeListOf<HTMLButtonElement>;
	const passwordSpans: NodeListOf<HTMLSpanElement> = doc.querySelectorAll( '[data-password]' ) as NodeListOf<HTMLSpanElement>;
	...
	// ============ Copy action ============
	const copyButtons: NodeListOf<HTMLButtonElement> = doc.querySelectorAll( '[data-myopie="copyPassword"]' ) as NodeListOf<HTMLButtonElement>;
	t.is( 2, copyButtons.length, 'Must have two copy buttons' );
	copyButtons[ 1 ]!.dispatchEvent( new DomEvent( 'click', { bubbles: true } ) );
	await new Promise( ( resolve: ( value: void ) => void ): void => { setTimeout( resolve, 100 ); } );
